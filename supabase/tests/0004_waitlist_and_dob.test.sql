-- OhHi migration 0004 acceptance tests (pgTAP)
--
-- Covers the two gaps 0004 closes: public.request_waitlist(text) (decision
-- 34) and the users_private.date_of_birth owner update grant (decision 35).
-- Run with `supabase test db`. Assumes migrations 0001-0003 are applied;
-- builds its own fixtures rather than reusing 0002_rules.test.sql's or
-- 0003_edge_support.test.sql's.
--
-- Same helper/role-switch conventions as those two files. Note (as there):
-- throws_ok's 3-arg form treats the 3rd argument as an exact expected error
-- MESSAGE once the 2nd argument looks like a SQLSTATE, so only the 2-arg
-- (sql, sqlstate) form is used here.
...
-- Run one statement under a bare role (no jwt claim), for the anon path.
...
-- Run one statement as a given user and return its SQLSTATE, or 'none' if it
-- did not raise. Lets a refusal be asserted from inside the user's own role
-- without aborting the file. The failed subtransaction rolls the `set local
-- role` back on its own, so no reset is needed on the exception path.
...
-- All on clcillinois.edu, the seeded coming_soon campus.
-- =============================================================================
...
select pg_temp._run_as('00040000-0000-0000-0000-000000000001', $$select public.begin_signup()$$);
select pg_temp._run_as('00040000-0000-0000-0000-000000000002', $$select public.begin_signup()$$);
select pg_temp._run_as('00040000-0000-0000-0000-000000000003', $$select public.begin_signup()$$);
...
select pg_temp._run_as_role('anon', $$select public.request_waitlist('wl1@nowhere.edu')$$);
...
select pg_temp._run_as('00040000-0000-0000-0000-000000000001',
...
select pg_temp._run_as('00040000-0000-0000-0000-000000000003',
